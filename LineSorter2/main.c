...
void test_compare() {
	...
	assert(str_compare("nCNecPH-ahjU!psIOFB", "siNXcQWqqFAdynriZFVL,") == -1);
	...
	assert(str_compare("kBdN.qZYTWH?'pLFycDai", "nyZEgoeYZ..FpKIpOp") == -1);
	...
	assert(str_compare("NrvpWJO!nrtpqWRmpLqwok", "SEs'AD.ivCgVwHYPFqJVgKxC") == -1);
	assert(str_compare("?ApXdmeeD?", "W.tb-cyenQZoCdLnQHVBdBTm") == -1);
	assert(str_compare("IFOuIn", "SCbdY.Anik!Vncnm!,!wyc t") == -1);
	assert(str_compare("CfDcJkgaIZtsahEdo!DAbAOc", "HKXHCclAFeeQzxA?sLomqHJ") == -1);
	assert(str_compare("oswjAnHYMMaSXpfVhlEeAU", "z!EQy,xekEfvBBXPQJWAn") == -1);
	...
	assert(str_compare("gltMsvfg uyygeKJ!dPOvF", "XjVhqsFjG!DBBBdtthggxRj") == -1);
	...
	assert(str_compare("nktoWnfioYhauh.wM' kW'H", "ycMX-rhrYIiFyY.jLEY-ONM") == -1);
	assert(str_compare("ahXINknIgnFzDFYznfeXy", "ZG.f?kAqpXamIVjNk-pJOIc") == -1);
	...
	assert(str_compare("k.EKZuQ!XCEhtvgqQnSYei", "srbRu.dVjOHBlY'ZTJS rJm") == -1);
	assert(str_compare("?BmjucGycg?FGMPb-bj?wx-a", " pcOBABNYzoHH.CisVX.LQ") == -1);
	assert(str_compare("NASM?AuqZYOFoETKVJ", "QOGlHMiILMqyA-JgXKN.LG") == -1);
	...
	assert(str_compare("Fnfc'mqRQ Fu?WWXGNvh", "gIAQ uT-oPeMqhfqLfT") == -1);
	...
	assert(str_compare("eBYQmkRBrIc.'IlC!iMxv", "JmcRO!ujMDlJaBdYziO?U") == -1);
	assert(str_compare("aNLfeGImWrN?HVi!ZNcUys", "VST'jYIVEKFf?H!OsmDr") == -1);
	...
	assert(str_compare("DvDGPKfDLsFZdoc,F", "IeS!s,h?og-qZlJo?ECZZWO") == -1);
	...
	assert(str_compare("NgjfAvSDwsVZuf,zA?mvw", "T?fqXBYCpsJWU?t DKw,") == -1);
	...
	assert(str_compare("mqLHdkXpaTrGPe,pvlDP", "qVAW.wiaZ?ujwN!kdLTWxIo'") == -1);
	assert(str_compare("prxVDxHzMOmtLhy-YuNTWr", "z!TKPDKjL,zyXrmEvv") == -1);
	...
	assert(str_compare("mvkIipkjxaVYgzyIFm,SbBRO", "ZTSjLhiaHnK,Dya ") == -1);
	assert(str_compare("''UzAFzJZwjHjn", "WigmXbFAhlgt") == -1);
	assert(str_compare("qq'qwEx", "SdFmeLH") == -1);
	assert(str_compare("sskRHt,cHfK'cb,", "vW,QEQXsDTAHzvRmSZJoYXu?") == -1);
	assert(str_compare("GmLobsFkDpKR", "nvwyoxWJBaEUvtxEw.'Zv") == -1);
	assert(str_compare("UKJTIF", "ySb'nIdDMUuYmheU!peKAtP") == -1);
	assert(str_compare("tHbSsgMmby-!jKOLiK aqCd", "Di UXuo-h-l.uAVYy-Trh") == 1);
	assert(str_compare("wmmtGm'.or'GBeBLcV", "og-LoT,rw?vzJbLoEmSnvXUl") == 1);
	assert(str_compare("RFctcLjfz", "qDObdjr?I-CvOJzbjQFBCCbY") == 1);
	assert(str_compare("?!LakV.MdafhsN", "h f,HUker?M-LzT CW") == 1);
	assert(str_compare("WQFNxPDxV-P-CdbIYWCR", "dSCsumSN?PRRipxWZ MD ") == 1);
	...
	assert(str_compare("yDSMfCJOVmMTVqqtECrgr", "L?ujTSUccRTtdQkqp?.D") == 1);
	...
	assert(str_compare("WbPt.Nqb.WEjXctSd!JdgH", "GgOVqowceUVHSHSHaOfJNyp-") == 1);
	assert(str_compare(".RSsslTBsTrk'pBL'YY", "IfZoYEqAS br?.GdptJrz") == 1);
	...
	assert(str_compare("t nLgGxRYzPIDF!Ik.ACuC", "iSHjzo'rHweuBjXaZvOmwk") == 1);
	...
	assert(str_compare("Wzf.RufSs,K'F.JFRzzUQU", "?.nyYfGlcdSOpsLvV'jU") == 1);
	...
	assert(str_compare("qflsJuSbhtcsV-icoDPdEhCO", "P-LRKiyLcnqoOJABX!wF,dw") == 1);
	...
	assert(str_compare("xFUXakWKIJWaxADBl.Ey", "-wG,Svq,fJuOvDcUwVcOP") == 1);
	...
	assert(str_compare("V kzMUv!fbQQSv CpFpp.H?", ".etNauIfqAPzEoXYmg!M'c") == 1);
...
void test_rev_compare()
{
	assert(str_rev_compare("fsHLgtz R,gZOF.XaChH", "EixBvO,tkJaY!Mx") == -1);
	...
	assert(str_rev_compare("ADTASy,q PJczFR,aIt'FB!", "-iU,h,YLPahdVx") == -1);
	...
	assert(str_rev_compare("enY?JTHgoNtLn?DrdnARe H", "HHGfvDgGHFjDQXcG dBK") == -1);
	assert(str_rev_compare("WVr'mS!EzKgt!", "SKdvm'JobTRoIZelY,Bx?") == -1);
	assert(str_rev_compare("IasvTRHtdcvfBVgyMNKCb", "MHwr!kktVqHrspOLNJwdy") == -1);
	...
	assert(str_rev_compare("QtCWH?lHWJkEGFgkx,c.FI", "oxj-i!Uct R'W") == -1);
	...
	assert(str_rev_compare("lKsQ-s-'MJrfVg.TR", "NJ!vKS EF'.kMLPPqiY") == -1);
	assert(str_rev_compare("tq'Fjeh", "nMBBDJd'p") == -1);
	assert(str_rev_compare("nNEcwBouVXWVYhB!uAiF", "PLH'tEJBhM!FXQy") == -1);
	assert(str_rev_compare("p?tNlSrxhByHSIieiIRFs", "mae'lsQPQpV") == -1);
	assert(str_rev_compare("AS'Kn-TbzURL!J", "PrTA?C'F'sazJcws") == -1);
	assert(str_rev_compare(".'ck-'ZlIhiYeCOU", "nPnmjxGQGbMZOMWWM.BXVPyv") == -1);
	assert(str_rev_compare("Yo'W,Lizc", "FlJbUUFaKGL,") == -1);
	assert(str_rev_compare("jYszjlQrN-vmrlQyDiwYAo", "RBgeEWlTHDu? ") == -1);
	...
	assert(str_rev_compare("FrDyhp P'L.Yj?yxBJfj-", "LGNwRXvjd,qhSyT") == -1);
	assert(str_rev_compare("QTiJG.JZPkSZfXBpsPgaoEc", "YGKhddOvJuTmNbfStOz,CuK?") == -1);
	...
	assert(str_rev_compare("ZPyx,jMDuomeyhDXP'zKqA", "!rFgiBXcBU?-?'h.OQ") == -1);
	...
	assert(str_rev_compare("wFqBKR' PvoTXQ", "m?gCqmP.or!!vD-pfq,kBS") == -1);
	assert(str_rev_compare(" quRxsQjpOxaH", "BBJIJVi.CW'SxmJ") == -1);
	assert(str_rev_compare("Cie!HQpoJbXiSEeWjPg", "pCswhiDPSQCKg'ZfSdQB ,As") == -1);
	assert(str_rev_compare("frQVkF", "GRG'GnAvNnuyn") == -1);
	assert(str_rev_compare("ud,'jCSWxMLWvSZe", "zzJhti'Y!cS") == -1);
	assert(str_rev_compare(" ViG??bY,zwMTvTateHf-Ck", "NkFRiiz.aoq'") == -1);
	assert(str_rev_compare("zZ'DXCaplnkxuOx M", "sjULbKtj kx-T") == -1);
	assert(str_rev_compare("oLINXctpikJatwHOWIojHA", "FkbhlpiTajhLsuah") == -1);
	assert(str_rev_compare("EhJFotmD", "GSPblDWvgc.Xxi") == -1);
	assert(str_rev_compare("'bO!FEQdCp.'kvO!thcQ,", ",,fhXtA!La!pL?.kWau") == -1);
	assert(str_rev_compare("e!IH'S?cy-h dPBkjC", "eQNhsCfRzbFqOH!oW,Znh") == -1);
	assert(str_rev_compare("P,AsK!WAw!YjpzRJJ?QJYfGF", "GOTa,mOIxhVogxVzlxP") == -1);
	assert(str_rev_compare("azQiXpKtBWLE", "!-WEIy.,-wdhN") == -1);
	assert(str_rev_compare("qKGfTcUmkHq'sHZAB", "gSPygOTTWaNsyNd.URb,e.") == -1);
	assert(str_rev_compare(".hqFwHRhzumSLqpjgq", "Ex'LkElHQmOZ") == -1);
	assert(str_rev_compare("g.V crPojQzq'Nrlg", "ZgqYb-AgoQ Aub-rf,EjOHUy") == -1);
	...
	assert(str_rev_compare("GAUag.XXYJDBdJj", "ODLh?'SrBJzqP !-LOY-.n") == -1);
	...
	assert(str_rev_compare("vqzNQ rEeD-MCpva?G Jr", "zdsiR'BPi'esj.Jzqs") == -1);
	assert(str_rev_compare("o'LfpPI,iUCsbgTbHiC,hSYp", "cxNIWXXPJY.?iYNztFOE,y'") == -1);
	...
	assert(str_rev_compare(".h.B Uaf''iplyfqSsX", "lWpOaxdwLNKbCtjDiV.x") == -1);
	assert(str_rev_compare("W sAdsgErAqdsaUnjkzqLste", "Bh-XP") == -1);
	assert(str_rev_compare("loYICd?Fq?tdzCc", "WKJGIzBuUCn,") == -1);
	assert(str_rev_compare(" A?idT nYKRUCH?Ai", "nxBp,attX'UGOsKhd-W") == -1);
	assert(str_rev_compare("eQNoGiGA!edVNnI", "D,HxffVHnQKEM'S") == -1);
	assert(str_rev_compare("pFs,NMT.-CJBdxUui.nxJuVf", ",ka-' !yDp RyObtv") == -1);
	assert(str_rev_compare("OTPLSRprjxMIggKfg", "Jszmnv'OwbJWIZuovMN'y!j") == -1);
	assert(str_rev_compare("KujqSeCrahhbfp?yaTDK", "d?ObgJmAkElyDOykG?n'fF") == 1);
	...
	assert(str_rev_compare("i.WOXt.SEoPINNQPmwB,bM!", "GyhGE,LUwDtPRoIiAHJ") == 1);
	...
	assert(str_rev_compare("lvcEeVPwdHQ?nH,shE?imsZ'", "kAB.cJPLpwkhB.kiDKtt") == 1);
	assert(str_rev_compare("rfCxxWpRYDYNEiiny", "DsuWD,fEU") == 1);
	assert(str_rev_compare("IJ,oY.IdY v'OI-j", "w.YlrBvmYULclbW'uc") == 1);
	assert(str_rev_compare("zH -LmNnrXGtKjWNu", "BjSvwrLnOZPw,lnGczRXa") == 1);
	assert(str_rev_compare("wD V-QGwx?JX NYWm", "krQjmN?XgQSmmUTtPxquyyQe") == 1);
	...
	assert(str_rev_compare("IUVZ.RBG?Ro?ngUTnGC TTDr", "qOI-YR aiM'qvBLbb.") == 1);
	assert(str_rev_compare(" sjDwqzkSKeDesYTbCPz", "QzUMmdjEHOuLXQnCW") == 1);
	assert(str_rev_compare("kDsk-'AJ", "tIbHHn.A'pkb.RDtQoR-,X?h") == 1);
	assert(str_rev_compare("IRpadlKFtTt?iwsEnt ipRhJ", "npImnoD!SVf,") == 1);
	...
	assert(str_rev_compare("'.GZDw.QA,PF'WWhhKxq", "NGMgVvbgeiE!qfCFl") == 1);
	assert(str_rev_compare("-DZPOli,I-zSNqTpV?Ui FFw", "K.VbjIDsdENC") == 1);
	...
	assert(str_rev_compare("nspLR-HAj.YvQnJiQowi.yDR", "MI'zvzRXODYySkH-af") == 1);
	assert(str_rev_compare("WmJyXnt?JmJEOOZ", "NUayYkCY!YNwIK-ssn,xsbRf") == 1);
	assert(str_rev_compare("'DITmRtFOF!vh'!aeU,?", "CpQJTSuq") == 1);
	assert(str_rev_compare("aTIlpHI!NcZe", "!FKxst,KzXzRFA") == 1);
	assert(str_rev_compare("yvnh?rSaLdpPI JPDQw-T", "-D,ZenQJPnCe.rt'Gg") == 1);
	...
	assert(str_rev_compare("LAfGhAmz!fDXZBz-k", "YjG.mvLFA DUq,vAii?CI") == 1);
	assert(str_rev_compare("rydyDbHYhoNOn.TFOr p-", " '!wYOb?eZyo!aNm'vL") == 1);
	assert(str_rev_compare("zIi,fH!j-?zM-x", "bIKnFRRMYKJwGuXBzubT") == 1);
	assert(str_rev_compare("yCBdTY?.X", "odvgyi.cKwSbR") == 1);
	assert(str_rev_compare("cLiajiaRwWMltizI onl'bjk", "UytSBH,hTyTXf") == 1);
	...
	assert(str_rev_compare("zLM B,s,i,Twr,z", "gLOzSj-PUBvaOxssFqiJB") == 1);
	assert(str_rev_compare("Vf.bhqkOgifDLIS", "ciYKcuoIkMSmFEYScLOLtPiD") == 1);
	...
	assert(str_rev_compare("NEycKibtpTzkT,dv?JwmlZ", "hnnh--mL,xk Jysaf") == 1);
	assert(str_rev_compare("aadeIL'XRRqPmR", "eGUeAVIODeGa'RHN-.yo") == 1);
	assert(str_rev_compare("Mu ?g Gh-hJxBNx!y'Fmksx", "knc,.lyqss dRe?L?dEv") == 1);
	assert(str_rev_compare("Duwy,vYRM!TKTtRUv", "FubdE,hwXrcFoilg") == 1);
	assert(str_rev_compare("tKUU'fAXLFdykH ", "WOUC-lUuEQiHJ.sr!-FO-.d") == 1);
	assert(str_rev_compare("f,aau edyTMyv!CirlefQ?p", "Wcqej!") == 1);
	assert(str_rev_compare("!BBc-V.huXeL?xqOJV", "XYybw?GsHiuRltbpRbebJfgV") == 1);
	assert(str_rev_compare("aeB?NHU", "qyOeBBKe'kxtZII") == 1);
	assert(str_rev_compare("wM,hZ", "'Y'!c..rT?dy ") == 1);
	assert(str_rev_compare("jYJ!FFGLQJPfkveksKnmW", "VBundAfTdLNCWVUo") == 1);
	...
	assert(str_rev_compare("N!ErEa YMiwBD,nyKfNSYVrg", "Q.MIIrOnwiURcYygJa") == 1);
	...
	assert(str_rev_compare("RBShUUI'QuPJ'NhMDEzXP.", "FWqSiQglnKfe ZD") == 1);
	...
	assert(str_rev_compare("d,FmXvnXSIybaXyjSxY!J", "BGBKmRAP'bCBofGTHYdxSPTE") == 1);
